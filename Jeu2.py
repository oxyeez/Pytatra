...
'''
def cree_replay(jeu):
	(jeu[1], jeu[2], jeu[3]) = (Pile.cree(), [Joueur.cree(1), Joueur.cree(2)], {'indiceJoueur' : 0})
'''

def gameSaved(jeu, pile, joueurs, indice):
	(jeu[1], jeu[2], jeu[3]) = (pile, joueurs, indice)
	return jeu
	#fonction que j'ai rajouté afin de modifié la liste contenant les informations du jeu en fonction des informations qui sont sauvegardées
...
def joueurCourant(jeu):		#retourne les informations du joueur qui est en trin de jouer
	return jeu[2][indiceJoueur(jeu)]

def passeJoueurSuivant(jeu):	#change l'indice du joueur qui est en train de jouer
	if indiceJoueur(jeu) == 0 :		#si l'indice du joueur est 0, on le remplace par 1
		jeu[3]['indiceJoueur'] = 1
	else :							#sinon, c'est que l'indice est 1, on le remplace donc par 0
...
def joue(jeu):
	...
	if os.path.getsize('GameSave.txt') != 0 :
		if Dialogue.choix('Il existe une partie sauvegardée.\nVoulez-vous la continuer ?') :
	...
	if Desequilibre :
		...
		#replay_choice = Dialogue.choix('Voulez-vous revoir la partie ?')
	...
	'''
	if GameOn and replay_choice :
		replay(jeu)
	else :
		play_again = Dialogue.choix('Voulez-vous refaire une partie ?')'''
	Fenetre.quitte(fenetre(jeu))
	
	return Dialogue.choix('Voulez-vous refaire une partie ?')
...
def activite(jeu):		#effectue toutes les actions qui doivent etre effectuées durant un tour
	...
	le return permet de bloquer la fonction activite()
	'''
	...
	if decalage == None :
		...
		passeJoueurSuivant(jeu)
		return
	#replay_data.append(planchetteAPoser)
	#replay_data.append(decalage)
	...
	passeJoueurSuivant(jeu)
	'''
	afin que, lorsque l'on met fin à la partie au moment de choisir un décalage et que l'on sauvegarde, il n'y ait pas une planchette qui soit retirée de la pioche mais pas ajoutée dans la pile, 
	on ne retire la planchette de la pioche qu'une fois le tour terminé (on est obligé de changer de joueur avant, et après cela)
	'''
...
def selectionnePlanchette(jeu):
	...
	'''
	tant que la valeur entrée n'est pas un numéro de planchette qui est contenu dans la pioche du joueur qui est en train de jouer ou 
		que None n'est pas affectée à la variable (c'est dans le cas où le joueur choisi cancel au lieu de choisir une valeur)
	une fois sorti de la boucle, si la variable contient None, on return None, 
		mais si elle contient un numéro de planchette, on retourne l'exemplaire contenant la planchette choisi
	'''
...
def choisisDecalage(jeu, planchetteAPoser):
	...
	lorsque le décalage vérifie les conditions, on retourne cette valeur de décalage
	si le joueur choisi cancel et ne rentre donc pas de décalage, None est retourné par la fonction siasiFlottant, dans ce cas on casse la boucle (break)
	'''
'''
def replay(jeu) :
	global step
	...
	cree_replay(jeu)
	majVues(jeu)
	but_next = VueBoutons.set_but(Fenetre.tk(fenetre(jeu)), 'Suivant', lambda : next(jeu))
	but_quit = VueBoutons.set_but(Fenetre.tk(fenetre(jeu)), 'Quitter', lambda : leave(jeu))


def next(jeu) :
	global step
	global but_next
	Pile.empileEtCalcule(pile(jeu), replay_data[step], replay_data[step+1])
	Pioche.retire(Joueur.pioche(joueurCourant(jeu)), Planchette.numero(replay_data[step]))
	passeJoueurSuivant(jeu)
	majVues(jeu)
	step+=2
	if step == len(replay_data) : 
		VueBoutons.unset_but(but_next)
